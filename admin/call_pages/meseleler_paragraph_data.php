...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT id, nomeri, Bolum_ady, Paragraf_no, Paragraf_ady, PDF_file_ady, Surat FROM meseleler_data WHERE Paragraf_ady='$paragraf_ady' ";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<div class="col-md-2"><label>Mesele (ýumuş) No</label><input type="text" name="Bolum_belgi[]" value="' . htmlspecialchars($row['nomeri']) . '" class="form-control"></div>';
...
<script>
    $(function() {
        $(".update-row").click(function() {
            ...
            formData.append("amaly_upd", "meseleler_upd");
...
if (response.PDF_file_ady) {
    iframe.attr("src", "call_pages/pdf_files/" + response.PDF_file_ady + "?t=" + new Date().getTime());
        ...
        //   .fadeIn().delay(5000).fadeOut();
          ...
          setTimeout(function () {           
                    var page="call_pages/meseleler_data.php";//amaly_data.php";
                    $(".main_place").empty()
        ...
        $(document).on("click", ".delete-row", function(event) {
            ...
            var mesele_delete = "meseleler_data";
